...
export function InteractiveSkateboard({deckTextureURL, wheelTextureURL, boltColor, truckColor}: Props) {
  return (
    <div className='absolute inset-0 z-10 flex items-center justify-center text-re'>
...
function Scene({deckTextureURL, wheelTextureURL, boltColor, truckColor}: Props) {
...
function ollie(board:THREE.Group){
  jumpBoard(board)
    ...
    .timeline()
    .to(board.rotation, {
      x:-.6, duration: 0.26, ease: 'none'
    })
    .to(board.rotation, {
      x:0.4, duration: 0.82, ease: 'power2.in'
    })
    .to(board.rotation, {
      x:0, duration: 0.12, ease: 'none'
    })
  }
  ...
  function kickflip(board:THREE.Group){
    jumpBoard(board)
      ...
      .to(board.rotation, {
        z: `+=${Math.PI * 2}`, duration: .78, ease:"none"
      }, 0.3)
      .to(board.rotation, {
        x:0, duration: 0.12, ease: 'none'
      })
...
<mesh position={[0, 0.27, -0.9]} name="back" onClick={onClick}>
  ...
  <meshStandardMaterial visible={false}/>
      ...
      scale={[0.2, 0.2, 0.2]}
      >
        <Html transform zIndexRange={[1, 0]} occlude="blending" wrapperClass="pointer-events-none">
          <WavyPaths />
        </Html>
      </group>
    </group>
